...
export class ScenePreload {
	...
	nextSceneName = "map1";
	preloadScenes = ["info", "dialog", "map1"];
		...
		preload.loadPacks(this.preloadPackFiles, () => {
			...
			scene.game.loadScenes(this.preloadScenes, () => {
				...
				if (needClickToContinue) {
					scene.components.event["onPointerdown"] = () => scene.game.scene = scene.game.scenes[this.nextSceneName];
					message.get("display").object.text += " (click here to continue)";
				} else scene.game.scene = scene.game.scenes[this.nextSceneName];
			}, (percentage) => {
